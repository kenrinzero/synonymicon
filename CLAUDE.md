...
- **Corpus frequency tables** — `data/subtlex_us.xlsx` (SUBTLEX-US, Brysbaert & New 2009), `data/bnc_all.al` (BNC, Kilgarriff, ~939k lemmas), `data/google_1grams.txt` (Norvig, 333k words), `data/wikipedia_freq.txt` (IlyaSemenov 2023, 2.77M words), `data/kaggle_freq.csv` (rtatman, 333k words), `data/hermitdave_freq.txt` (hermitdave 2018, 1.66M words), and `data/scriptsmith_freq.txt` (scriptsmith, 3.08M words) loaded at startup alongside wordfreq. `get_zipf(word, corpus)` dispatches to the selected source. SUBTLEX-US Zipf values are pre-computed; BNC is `log10(count × 1B / 85_714_226)`; Google/Kaggle use `log10(count) + 3.0`; Wikipedia uses `log10(count) - 0.5`; hermitdave/scriptsmith use `log10(count) + 0.37`.
...
- `data/hermitdave_freq.txt` — hermitdave FrequencyWords (1.66M words), loaded at startup
- `data/scriptsmith_freq.txt` — scriptsmith topwords (3.08M words), loaded at startup
...
Valid `corpus` values: `wordfreq` (default), `subtlex` (SUBTLEX-US film subtitles), `bnc` (British National Corpus, lemmatized), `google_1grams` (Norvig), `wikipedia` (Wikipedia 2023), `kaggle` (rtatman), `hermitdave` (FrequencyWords 2018), `scriptsmith` (topwords). Controls which frequency table is used for Zipf filtering. Unknown values return 400 with `available_corpora` list.
